...
const char* ssid = "Surveillance Drone202";
  ...
  while (!client.connected()) {
    ...
    if (client.connect(clientId.c_str(),"openhabian", "openhabian")) {
     ...
     // client.publish("outTopic", "hello world");
      // ... and resubscribe
      client.subscribe("/home/esp8266/switchBed/command");
    } else {
...
void loop() {
  ...
  if(digitalRead(input1) == HIGH){
          digitalWrite(load1, HIGH);
          client.publish("/home/esp8266/switchBed/state","1");          
  } else if(digitalRead(input1) == LOW){
          digitalWrite(load1, LOW);
          client.publish("/home/esp8266/switchBed/state","0");
  }
  delay(500);
  client.loop();
}
